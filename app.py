...
age_range = st.sidebar.slider("Select Age Range", int(df.CustomerAge.min()), int(df.CustomerAge.max()), (20, 50))
gender_filter = st.sidebar.multiselect("Select Gender", options=df.Gender.unique(), default=list(df.Gender.unique()))
location_filter = st.sidebar.multiselect("Select Location", options=df.Location.unique(), default=list(df.Location.unique()))
income_filter = st.sidebar.multiselect("Select Income Level", options=df.IncomeLevel.unique(), default=list(df.IncomeLevel.unique()))
loyalty_filter = st.sidebar.multiselect("Loyalty Member", options=df.LoyaltyMember.unique(), default=list(df.LoyaltyMember.unique()))
category_filter = st.sidebar.multiselect("Product Category", options=df.ProductCategory.unique(), default=list(df.ProductCategory.unique()))
